...
module.exports = async function roleUpdate(client, guild, audit, member, changes, ares) {
	const safeMode = await checkWhitelist(client, member, 'role');
	if (safeMode?.isWarn) return;
	if (safeMode?.isWarn && !ares.blackListedRoles.includes(audit?.targetId)) return;
