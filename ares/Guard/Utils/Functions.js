...
async function punish(client, member, type, reason) {
	...
	switch (type) {
		case 1:
			member.guild.bans.create(member.id, { reason: 'ares Security - ' + reason }).catch(() => { });
			break;

		case 2:
			member.kick('ares Security - ' + reason).catch(() => { });
			break;
...
async function checkWhitelist(client, member, type) {
	...
	const whitelistType = guildOwner ? '👑 Taç Sahibi ' : fullAccess ? '🟢 Full Erişim' : developers ? `👑 Developer` : bots ? `🤖 ares Bots` : `🟢 Güvenli Listede`
