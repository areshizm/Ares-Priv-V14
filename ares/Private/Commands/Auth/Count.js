const { PermissionsBitField: { Flags }, EmbedBuilder, bold, inlineCode ,ChannelType} = require('discord.js');
...
module.exports = {
    ...
    execute: async (client, message, args, ares) => {
       ...
       const voiceChannels = message.guild.channels.cache.filter(c => c.type === ChannelType.GuildVoice);
        ...
        const backgroundBuffer = await loadImage(path.resolve(__dirname, '../../../', 'Server.png'));
        ...
        const datas = [
            ...
            { name: 'Ses Sust. Sayısı:', value: members.filter(m => m.voice.serverMute).size, svg: 'mute' },
              { name: 'Aktif Kanallar:', value: voiceChannels.filter(channel => channel.members.size > 0).size, svg: 'channel' },
